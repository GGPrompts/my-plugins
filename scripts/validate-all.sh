...
# Quick validation of all plugins before commits
#
...
# Target directory (default to all plugins)
TARGET_DIR="${1:-$PROJECT_ROOT/plugins}"

echo -e "${CYAN}============================================${NC}"
echo -e "${CYAN}     MY-PLUGINS VALIDATION SUITE${NC}"
echo -e "${CYAN}============================================${NC}"
echo ""
...
done < <(find "$TARGET_DIR" -path "*/.claude-plugin/plugin.json" -type f 2>/dev/null)
...
done < <(find "$TARGET_DIR" \( -path "*/commands/*.md" -o -path "*/agents/*.md" \) -type f 2>/dev/null | head -100)
...
done < <(find "$TARGET_DIR" -name "SKILL.md" -type f 2>/dev/null)
...
done < <(find "$TARGET_DIR" -name "*.sh" -type f 2>/dev/null)
...
# Count totals
plugin_count=$(find "$TARGET_DIR" -path "*/.claude-plugin/plugin.json" -type f 2>/dev/null | wc -l)
skill_count=$(find "$TARGET_DIR" -name "SKILL.md" -type f 2>/dev/null | wc -l)
command_count=$(find "$TARGET_DIR" -path "*/commands/*.md" -type f 2>/dev/null | wc -l)
agent_count=$(find "$TARGET_DIR" -path "*/agents/*.md" -type f 2>/dev/null | wc -l)
