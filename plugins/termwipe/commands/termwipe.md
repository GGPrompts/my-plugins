---
description: Generate handoff, clear context, and auto-continue in fresh session (Termux-optimized)
---
...
```bash
# Save handoff to temp file (Termux-compatible)
HANDOFF_FILE="${TMPDIR:-${PREFIX:+$PREFIX/tmp}}"
HANDOFF_FILE="${HANDOFF_FILE:-/tmp}/claude-handoff-$$.txt"
...
# Copy to clipboard as backup (Termux first)
if command -v termux-clipboard-set &> /dev/null; then
    cat "$HANDOFF_FILE" | termux-clipboard-set
    echo "Backup copied to clipboard (Termux)"
