...
**Do not use manual bullet symbols (•, -, *, etc.)** - Use `<ul>` or `<ol>` lists instead
...
- **Do not use CSS gradients (`linear-gradient`, `radial-gradient`)** - They don't convert to PowerPoint
- **Create gradient/icon PNGs first using Sharp, then reference in HTML**
...
#### Colors
- **Do not use `#` prefix** with hex colors in PptxGenJS - causes file corruption
